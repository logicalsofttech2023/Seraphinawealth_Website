...
const MyPlan = () => {
  ...
  const fetchUserPlan = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setCurrentPlanIndex(0); // Reset to first plan when data loads
      } else {
        setPlans([]);
      }
    } catch (error) {
  ...
  const renderRenewButton = (plan) => (
    ...
    >
      <button
        onClick={() => handleRenewPlan(plan._id)}
        disabled={renewing}
        style={{
  ...
  const renderPlanDetails = () => {
    ...
    return (
      <div style={{ marginTop: "24px" }}>
        {plans.length > 1 && renderPlanNavigation()}
        ...
        <div key={plan._id} style={{ marginBottom: "40px" }}>
          ...
          >
            ...
            >
              {plan.serviceChoice.charAt(0).toUpperCase() +
                plan.serviceChoice.slice(1)}{" "}
              Plan
            </h3>
            <span
              style={{
            ...
            >
              ...
              <div style={{ marginBottom: "20px" }}>
                {[
                  ...
                  { label: "Total Services", value: getServiceCount(plan) },
                  {
                    label: "Total Price",
              ...
              {plan.status === "expired" && renderRenewButton(plan)}
            </div>
...
const ServiceSection = ({ title, services, color, icon }) => (
